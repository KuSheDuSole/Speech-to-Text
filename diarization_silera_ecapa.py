...
def diarize(audio_path, min_speakers=1, max_speakers=4,
    ...
    if best_score < 0.10:
        print("⚠ silhouette low for all k — falling back to 2 clusters (but continue postprocessing).")
        best_k = 2
    ...
    if tiny_speakers:
        print(f"🧹 tiny speakers (total<{small_speaker_total_thresh_s}s): {tiny_speakers}. Reassigning their segments.")
        for seg in merged_segments:
            if seg["speaker"] in tiny_speakers:
                ...
                if prev_idx is not None and next_idx is not None:
                    ...
                    chosen = merged_segments[prev_idx]["speaker"] if gap_prev <= gap_next else \
                    merged_segments[next_idx]["speaker"]
                elif prev_idx is not None:
                    chosen = merged_segments[prev_idx]["speaker"]
                elif next_idx is not None:
                    chosen = merged_segments[next_idx]["speaker"]
                else:
                    chosen = "speaker_0"
                seg["speaker"] = chosen
        ...
        if dur < short_segment_thresh_s and 0 < i < len(merged_segments) - 1:
            ...
            if prev["speaker"] == nxt["speaker"]:
                ...
                for lab, cent in centroids.items():
                    new_lab = merged_map.get(lab, lab)
                    speaker_centroids[f"speaker_{new_lab}"] = cent
                c_prev = speaker_centroids.get(sp_prev, None)
                c_next = speaker_centroids.get(sp_next, None)
                ...
                if c_prev is not None and c_next is not None:
                    ...
                    d_prev = 1.0 - np.dot(emb_vec, c_prev)
                    d_next = 1.0 - np.dot(emb_vec, c_next)
                    seg["speaker"] = sp_prev if d_prev <= d_next else sp_next
                else:
                    seg["speaker"] = prev["speaker"]
    ...
    normalized = merge_adjacent(normalized, gap_threshold=0.30)
